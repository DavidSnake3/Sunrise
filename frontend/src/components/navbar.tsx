...
import {
  ...
  NavbarMenuItem,
  Button,
  CircularProgress, Dropdown, DropdownTrigger, DropdownMenu, DropdownItem, Avatar, User
} from "@heroui/react";
    ...
    <>
      ...
      >
        ...
        <NavbarContent className="hidden md:flex gap-4" justify="center">
          {siteConfig.navItems
            .filter((item) => item.label !== "Administrativo" || (user && user.admin)) // Mostrar solo si el usuario es admin
            .map((item, index) => (
        ...
        <NavbarContent justify="end">
          {loading ? ( // Mientras se carga la autenticación, mostramos un mensaje
            <NavbarItem>
              <CircularProgress aria-label="Loading..." color={`${!isScrolled && !isMenuOpen ? "secondary" : "primary"}`} />
            </NavbarItem>
          ) : user ? (
            <div className="flex items-center gap-3 user">
              <Dropdown placement="bottom-start">
                ...
                <DropdownMenu aria-label="User Actions" variant="flat" disabledKeys={["user"]}>
                  <DropdownItem key="user" className="lg:hidden"><p className="font-bold">Hola, {user.nombre_completo.split(" ")[0]}</p></DropdownItem>
